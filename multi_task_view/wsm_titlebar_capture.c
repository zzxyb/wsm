...
static void render_tree(struct wlr_scene_tree *tree, int sx, int sy,
	...
	wl_list_for_each(node, &tree->children, link) {
		...
		/* A scene buffer may release its backing wlr_buffer after importing it.
		 * Reuse that renderer-owned texture when present; only textures imported
		 * specifically for this capture belong in our destruction list. */
...
bool wsm_titlebar_capture(struct wsm_titlebar_capture *capture,
	...
	*capture = (struct wsm_titlebar_capture){0};
	...
	if (target == NULL) {
		wsm_log(WSM_ERROR, "Could not allocate titlebar capture buffer");
		return false;
	}
	struct wlr_render_pass *pass =
