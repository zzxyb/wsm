...
static bool scene_buffer_point_accepts_input(struct wsm_scene_buffer *scene_buffer,
		double *sx, double *sy) {
	struct wsm_scene_surface *scene_surface =
		wsm_scene_surface_try_from_buffer(scene_buffer);
...
static void scene_buffer_unmark_client_buffer(struct wsm_scene_buffer *scene_buffer) {
...
static void surface_addon_destroy(struct wlr_addon *addon) {
	struct wsm_scene_surface *surface = wl_container_of(addon, surface, addon);
...
static void surface_reconfigure(struct wsm_scene_surface *scene_surface) {
	struct wsm_scene_buffer *scene_buffer = scene_surface->buffer;
	...
	if (width <= 0 || height <= 0) {
		wsm_scene_buffer_set_buffer(scene_buffer, NULL);
	...
	wsm_scene_buffer_set_opaque_region(scene_buffer, &opaque);
	wsm_scene_buffer_set_source_box(scene_buffer, &src_box);
	wsm_scene_buffer_set_dest_size(scene_buffer, width, height);
	wsm_scene_buffer_set_transform(scene_buffer, state->transform);
	wsm_scene_buffer_set_opacity(scene_buffer, opacity);
	wsm_scene_buffer_set_transfer_function(scene_buffer, tf);
	wsm_scene_buffer_set_primaries(scene_buffer, primaries);
	wsm_scene_buffer_set_color_encoding(scene_buffer, color_encoding);
	wsm_scene_buffer_set_color_range(scene_buffer, color_range);
	...
	if (surface->buffer) {
		...
		struct wsm_scene_buffer_set_buffer_options options = {
		...
		wsm_scene_buffer_set_buffer_with_options(scene_buffer,
...
static void handle_scene_buffer_outputs_update(
	...
	struct wsm_scene_surface *surface =
		wl_container_of(listener, surface, outputs_update);
	struct wsm_scene_outputs_update_event *event = data;
	struct wsm_scene *scene = wsm_scene_node_get_root(&surface->buffer->node);
...
static void handle_scene_buffer_output_sample(
	...
	const struct wsm_scene_output_sample_event *event = data;
...
static void scene_surface_handle_surface_destroy(
	...
	wsm_scene_node_destroy(&surface->buffer->node);
}
...
static void handle_scene_surface_surface_commit(
	...
	struct wsm_scene_buffer *scene_buffer = surface->buffer;
	...
	// the surface anyway.
	int lx, ly;
	bool enabled = wsm_scene_node_coords(&scene_buffer->node, &lx, &ly);
...
struct wsm_scene_surface *wsm_scene_surface_create(struct wsm_scene_tree *parent,
	...
	struct wsm_scene_surface *surface = calloc(1, sizeof(*surface));
	if (surface == NULL) {
		return NULL;
	}

	struct wsm_scene_buffer *scene_buffer = wsm_scene_buffer_create(parent, NULL);
	if (!scene_buffer) {
		free(surface);
		return NULL;
	}
