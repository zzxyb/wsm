...
static struct wsm_scene_subsurface_tree *subsurface_tree_from_subsurface(
		struct wsm_scene_subsurface_tree *parent,
...
static bool subsurface_tree_reconfigure_clip(
	...
	if (wlr_box_empty(&subsurface_tree->clip)) {
		wsm_scene_surface_set_clip(subsurface_tree->scene_surface, NULL);
		wsm_scene_node_set_enabled(&subsurface_tree->scene_surface->buffer->node, true);
		wsm_scene_node_set_position(&subsurface_tree->scene_surface->buffer->node, 0, 0);
		...
		wsm_scene_node_set_enabled(&subsurface_tree->scene_surface->buffer->node, intersects);

		if (intersects) {
			wsm_scene_node_set_position(&subsurface_tree->scene_surface->buffer->node, clip.x, clip.y);
			wsm_scene_surface_set_clip(subsurface_tree->scene_surface, &clip);
		}
...
static void subsurface_tree_reconfigure(
	...
	struct wsm_scene_node *prev = NULL;
	struct wlr_subsurface *subsurface;
	wl_list_for_each(subsurface, &surface->current.subsurfaces_below,
			current.link) {
		struct wsm_scene_subsurface_tree *child =
			subsurface_tree_from_subsurface(subsurface_tree, subsurface);
		if (prev != NULL) {
			wsm_scene_node_place_above(&child->tree->node, prev);
		}
		prev = &child->tree->node;

		wsm_scene_node_set_position(&child->tree->node,
	...
	if (prev != NULL) {
		wsm_scene_node_place_above(&subsurface_tree->scene_surface->buffer->node, prev);
	}
...
static void subsurface_tree_handle_surface_destroy(struct wl_listener *listener,
	...
	wsm_scene_node_destroy(&subsurface_tree->tree->node);
}
...
static void subsurface_tree_handle_surface_map(struct wl_listener *listener,
	...
	wsm_scene_node_set_enabled(&subsurface_tree->tree->node, true);
}
...
static void subsurface_tree_handle_surface_unmap(struct wl_listener *listener,
	...
	wsm_scene_node_set_enabled(&subsurface_tree->tree->node, false);
}
...
static struct wsm_scene_subsurface_tree *scene_surface_tree_create(
	struct wsm_scene_tree *parent, struct wlr_surface *surface);

static bool subsurface_tree_create_subsurface(
		struct wsm_scene_subsurface_tree *parent,
		struct wlr_subsurface *subsurface) {
	struct wsm_scene_subsurface_tree *child = scene_surface_tree_create(
...
static struct wsm_scene_subsurface_tree *scene_surface_tree_create(
	...
	subsurface_tree->tree = wsm_scene_tree_create(parent);
	if (subsurface_tree->tree == NULL) {
		goto error_surface_tree;
	}

	subsurface_tree->scene_surface =
		wsm_scene_surface_create(subsurface_tree->tree, surface);
	...
	wsm_scene_node_set_enabled(&subsurface_tree->tree->node, surface->mapped);
...
static struct wsm_scene_subsurface_tree *get_subsurface_tree_from_node(
	...
	struct wsm_scene_subsurface_tree *tree =
		wl_container_of(addon, tree, scene_addon);
	return tree;
}

static bool subsurface_tree_set_clip(struct wsm_scene_node *node,
		const struct wlr_box *clip) {
	if (!wsm_scene_node_is_tree(node)) {
		return false;
	}

	bool discovered_subsurface_tree = false;
	struct wsm_scene_subsurface_tree *tree = get_subsurface_tree_from_node(node);
	if (tree) {
		if (tree->parent == NULL) {
