...
static enum libinput_config_status handle_set_send_events(
...
static enum libinput_config_status handle_set_tap(
...
static enum libinput_config_status handle_set_accel_speed(
...
static enum libinput_config_status handle_set_rotation_angle(
	struct libinput_device *device, double angle) {
...
static enum libinput_config_status handle_set_natural_scroll(
	struct libinput_device *d, bool n) {
	...
	enum libinput_config_status status =
		libinput_device_config_scroll_set_natural_scroll_enabled(d, n);
	return status;
}

static enum libinput_config_status handle_set_left_handed(
	struct libinput_device *device, bool left) {
	...
	enum libinput_config_status status =
		libinput_device_config_left_handed_set(device, left);
	return status;
}
...
static enum libinput_config_status handle_set_middle_emulation(
	...
	if (!libinput_device_config_middle_emulation_is_available(dev) ||
		libinput_device_config_middle_emulation_get_enabled(dev) ==
			mid) {
	...
	enum libinput_config_status status =
		libinput_device_config_middle_emulation_set_enabled(dev, mid);
	return status;
}
...
static enum libinput_config_status handle_set_scroll_button(
	struct libinput_device *dev, uint32_t button) {
	...
	enum libinput_config_status status =
		libinput_device_config_scroll_set_button(dev, button);
	return status;
}
...
static enum libinput_config_status handle_set_scroll_button_lock(
	...
	if ((scroll & ~LIBINPUT_CONFIG_SCROLL_NO_SCROLL) == 0 ||
		...
		wsm_log(WSM_ERROR,
			"handle_set_scroll_button_lock redundantly!");
		return LIBINPUT_CONFIG_STATUS_INVALID;
	}
	wsm_log(WSM_DEBUG, "scroll_set_button_lock(%" PRIu32 ")", lock);
	enum libinput_config_status status =
		libinput_device_config_scroll_set_button_lock(dev, lock);
	return status;
}

static enum libinput_config_status handle_set_dwt(
	struct libinput_device *device, bool dwt) {
...
static enum libinput_config_status handle_set_dwtp(
	struct libinput_device *device, bool dwtp) {
	...
	enum libinput_config_status status =
		libinput_device_config_dwtp_set_enabled(device, dwtp);
	return status;
}

static enum libinput_config_status handle_set_calibration_matrix(
	struct libinput_device *dev, float mat[6]) {
	if (!libinput_device_config_calibration_has_matrix(dev)) {
		wsm_log(WSM_ERROR,
			"handle_set_calibration_matrix redundantly!");
		return LIBINPUT_CONFIG_STATUS_INVALID;
	}
	bool changed = false;
	float current[6];
	...
	if (changed) {
		wsm_log(WSM_DEBUG,
			"calibration_set_matrix(%f, %f, %f, %f, %f, %f)",
			mat[0], mat[1], mat[2], mat[3], mat[4], mat[5]);
		enum libinput_config_status status =
			libinput_device_config_calibration_set_matrix(dev, mat);
		return status;
	}
	return LIBINPUT_CONFIG_STATUS_INVALID;
}
...
struct wsm_input_device *wsm_input_device_create() {
	...
	if (!input_device) {
		wsm_log(WSM_ERROR,
			"Could not create wsm_input_device: allocation "
			"failed!");
		return NULL;
	}
	input_device->input_device_impl_wsm = &input_device_impl;
	input_device->scroll_factor = 1.0f;
	return input_device;
}
...
void wsm_input_device_destroy(struct wlr_input_device *wlr_device) {
	struct wsm_input_device *input_device =
		input_wsm_device_from_wlr(wlr_device);
	...
	free(input_device->mapped_output_id);
	free(input_device);
}
