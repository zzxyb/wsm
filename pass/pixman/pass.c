...
static bool render_pass_submit(struct wlr_render_pass *wlr_pass) {
	struct wsm_pixman_render_pass *pass = wsm_pixman_render_pass_from_render_pass(wlr_pass);

	wlr_buffer_end_data_ptr_access(pass->buffer->base);
...
static void render_pass_add_texture(struct wlr_render_pass *wlr_pass,
	...
	struct wsm_pixman_render_pass *pass = wsm_pixman_render_pass_from_render_pass(wlr_pass);
	struct wsm_pixman_texture *texture = wsm_pixman_texture_from_texture(options->texture);
	struct wsm_pixman_buffer *buffer = pass->buffer;

	if (texture->buffer != NULL && !wsm_renderer_begin_pixman_data_ptr_access(texture->buffer,
...
static void render_pass_add_rect(struct wlr_render_pass *wlr_pass,
	...
	struct wsm_pixman_buffer *buffer = pass->buffer;
	struct wlr_box box;
	wlr_render_rect_options_get_box(options, pass->buffer->base, &box);
...
struct wsm_pixman_render_pass *wsm_pixman_render_pass_from_render_pass(struct wlr_render_pass *wlr_pass) {
	assert(wlr_pass->impl == &render_pass_impl);
	struct wsm_pixman_render_pass *pass = wl_container_of(wlr_pass, pass, base);
	return pass;
}

struct wsm_pixman_render_pass *wsm_render_pass_begin_pixman_render_pass(
		struct wsm_pixman_buffer *buffer) {
	struct wsm_pixman_render_pass *pass = calloc(1, sizeof(*pass));
	if (pass == NULL) {
		return NULL;
	}
	...
	if (!wsm_renderer_begin_pixman_data_ptr_access(buffer->base, &buffer->image,
