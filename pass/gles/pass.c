...
static bool render_pass_submit(struct wlr_render_pass *wlr_pass) {
	struct wsm_gles2_render_pass *pass = wlr_gles2_render_pass_from_render_pass(wlr_pass);
	struct wsm_gles2_renderer *renderer = pass->buffer->renderer;
	struct wsm_gles2_render_timer *timer = pass->timer;
	bool ok = false;
	...
	if (pass->signal_timeline != NULL) {
		EGLSyncKHR sync = wsm_egl_create_sync(renderer->egl, -1);
		if (sync == EGL_NO_SYNC_KHR) {
			goto out;
		}

		int sync_file_fd = wsm_egl_dup_fence_fd(renderer->egl, sync);
...
out:
	...
	wsm_egl_restore_context(&pass->prev_ctx);

	wlr_drm_syncobj_timeline_unref(pass->signal_timeline);
	wlr_buffer_unlock(pass->buffer->base);
	free(pass);
...
static void render_pass_add_texture(struct wlr_render_pass *wlr_pass,
	...
	struct wsm_gles2_render_pass *pass = wlr_gles2_render_pass_from_render_pass(wlr_pass);
	struct wsm_gles2_renderer *renderer = pass->buffer->renderer;
	struct wsm_gles2_texture *texture = wsm_gles2_texture_from_texture(options->texture);

	struct wsm_gles2_tex_shader *shader = NULL;
...
static void render_pass_add_rect(struct wlr_render_pass *wlr_pass,
	...
	struct wlr_box box;
	struct wlr_buffer *wlr_buffer = pass->buffer->base;
...
struct wsm_gles2_render_pass *wsm_render_pass_begin_gles2_buffer_pass(struct wsm_gles2_buffer *buffer,
	...
	struct wsm_gles2_renderer *renderer = buffer->renderer;
	struct wlr_buffer *wlr_buffer = buffer->base;

	if (renderer->procs.glGetGraphicsResetStatusKHR) {
		GLenum status = renderer->procs.glGetGraphicsResetStatusKHR();
		if (status != GL_NO_ERROR) {
			wsm_log(WSM_ERROR, "GPU reset (%s)", reset_status_str(status));
			wl_signal_emit_mutable(&renderer->base.events.lost, NULL);
			return NULL;
		}
	}

	GLint fbo = wsm_gles2_buffer_get_fbo(buffer);
	if (!fbo) {
		return NULL;
	}

	struct wsm_gles2_render_pass *pass = calloc(1, sizeof(*pass));
	if (pass == NULL) {
		return NULL;
	}
...
bool wlr_render_pass_is_wsm_gles2(const struct wlr_render_pass *wlr_pass) {
	return wlr_pass != NULL && wlr_pass->impl == &render_pass_impl;
}
...
struct wsm_gles2_render_pass *wlr_gles2_render_pass_from_render_pass(
	...
	struct wsm_gles2_render_pass *pass = wl_container_of(wlr_pass, pass, base);
	return pass;
}
