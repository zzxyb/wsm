...
struct tinywl_server {
	...
	struct wsm_scene_output_layout *scene_layout;
...
static void focus_toplevel(struct tinywl_toplevel *toplevel) {
	...
	wsm_scene_node_raise_to_top(&toplevel->scene_tree->node);
...
static struct tinywl_toplevel *desktop_toplevel_at(
	...
	struct wsm_scene_node *node = wsm_scene_node_at(
		&server->scene->tree->node, lx, ly, sx, sy);
	if (node == NULL || !wsm_scene_node_is_buffer(node)) {
		return NULL;
	}
	...
	*surface = scene_surface->surface;
	...
	struct wsm_scene_tree *tree = node->parent;
...
static void process_cursor_move(struct tinywl_server *server) {
	...
	wsm_scene_node_set_position(&toplevel->scene_tree->node,
...
static void output_frame(struct wl_listener *listener, void *data) {
	...
	struct wsm_scene *scene = output->server->scene;

	struct wsm_scene_output *scene_output = wsm_scene_get_scene_output(
		scene, output->wlr_output);

	/* Render the scene if needed and commit the output */
	wsm_scene_output_commit(scene_output, NULL);

	struct timespec now;
	clock_gettime(CLOCK_MONOTONIC, &now);
	wsm_scene_output_send_frame_done(scene_output, &now);
}
...
static void server_new_output(struct wl_listener *listener, void *data) {
	...
	struct wlr_output_layout_output *l_output = wlr_output_layout_add_auto(server->output_layout,
		wlr_output);
	struct wsm_scene_output *scene_output = wsm_scene_output_create(server->scene, wlr_output);
	wsm_scene_output_layout_add_output(server->scene_layout, l_output, scene_output);
}
...
static void server_new_xdg_toplevel(struct wl_listener *listener, void *data) {
	...
	toplevel->scene_tree =
		wsm_scene_xdg_surface_create(toplevel->server->scene->tree, xdg_toplevel->base);
...
static void server_new_xdg_popup(struct wl_listener *listener, void *data) {
	...
	assert(parent != NULL);
	struct wsm_scene_tree *parent_tree = parent->data;
	xdg_popup->base->data = wsm_scene_xdg_surface_create(parent_tree, xdg_popup->base);
...
int main(int argc, char *argv[]) {
	...
	server.renderer = wlr_renderer_autocreate_wsm(server.backend);
	 ...
	 * necessary.
	 */
	server.scene = wsm_scene_create();
	server.scene_layout = wsm_scene_attach_output_layout(server.scene, server.output_layout);
	...
	wsm_scene_node_destroy(&server.scene->tree->node);
