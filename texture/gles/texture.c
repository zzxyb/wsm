...
static bool gles2_texture_update_from_buffer(struct wlr_texture *wlr_texture,
	...
	struct wsm_gles2_texture *texture = wsm_gles2_texture_from_texture(wlr_texture);
	...
	const struct wsm_pixel_format_info *drm_fmt =
		drm_get_pixel_format_info(texture->drm_format);
	assert(drm_fmt);
	if (pixel_format_info_pixels_per_block(drm_fmt) != 1) {
		wlr_buffer_end_data_ptr_access(buffer);
		wsm_log(WSM_ERROR, "Cannot update texture: block formats are not supported");
		return false;
	}
	...
	wsm_egl_make_current(texture->renderer->egl, &prev_ctx);

	wsm_gles2_push_debug(texture->renderer);
	...
	wsm_gles2_pop_debug(texture->renderer);
...
static bool gles2_texture_bind(struct wsm_gles2_texture *texture) {
	if (texture->fbo) {
		...
		GLuint fbo = wsm_gles2_buffer_get_fbo(texture->buffer);
		if (!fbo) {
			return false;
		}
...
static bool gles2_texture_read_pixels(struct wlr_texture *wlr_texture,
	...
	if (fmt == NULL || !is_gles2_pixel_format_supported(texture->renderer, fmt)) {
		wsm_log(WSM_ERROR, "Cannot read pixels: unsupported pixel format 0x%"PRIX32, options->format);
		return false;
	}

	if (fmt->gl_format == GL_BGRA_EXT && !texture->renderer->exts.EXT_read_format_bgra) {
		wsm_log(WSM_ERROR, "Cannot read pixels: missing GL_EXT_read_format_bgra extension");
		return false;
	}
	...
	assert(drm_fmt);
	if (pixel_format_info_pixels_per_block(drm_fmt) != 1) {
		wsm_log(WSM_ERROR, "Cannot read pixels: block formats are not supported");
		return false;
	}

	wsm_gles2_push_debug(texture->renderer);
	struct wsm_egl_context prev_ctx;
	if (!wsm_egl_make_current(texture->renderer->egl, &prev_ctx)) {
		return false;
	}
	...
	if (options->wait_timeline != NULL) {
		...
		struct wsm_gles2_renderer *renderer = texture->renderer;
		EGLSyncKHR sync = wsm_egl_create_sync(renderer->egl, sync_file_fd);
		close(sync_file_fd);
		if (sync == EGL_NO_SYNC_KHR) {
			return false;
		}

		bool ok = wsm_egl_wait_sync(renderer->egl, sync);
		wsm_egl_destroy_sync(renderer->egl, sync);
		if (!ok) {
			return false;
		}
	}
...
static void handle_gles2_texture_destroy(struct wlr_texture *wlr_texture) {
	wsm_gles2_texture_destroy(wsm_gles2_texture_from_texture(wlr_texture));
}
...
static struct wsm_gles2_texture *gles2_texture_create(
		struct wsm_gles2_renderer *renderer, uint32_t width, uint32_t height) {
	struct wsm_gles2_texture *texture = calloc(1, sizeof(*texture));
	if (texture == NULL) {
...
struct wsm_gles2_texture *wsm_gles2_texture_from_pixels(
	...
	if (fmt == NULL) {
		wsm_log(WSM_ERROR, "Unsupported pixel format 0x%"PRIX32, drm_format);
		return NULL;
	}
	...
	assert(drm_fmt);
	if (pixel_format_info_pixels_per_block(drm_fmt) != 1) {
		wsm_log(WSM_ERROR, "Cannot upload texture: block formats are not supported");
		return NULL;
	}
...
bool wlr_texture_is_wsm_gles2(const struct wlr_texture *wlr_texture) {
	return wlr_texture != NULL && wlr_texture->impl == &texture_impl;
}
...
struct wsm_gles2_texture *wsm_gles2_texture_from_buffer(struct wlr_renderer *wlr_renderer,
	...
	if (wlr_buffer_get_dmabuf(buffer, &dmabuf)) {
		return wsm_gles2_texture_from_dmabuf(renderer, buffer, &dmabuf);
	} else if (wlr_buffer_begin_data_ptr_access(buffer,
			WLR_BUFFER_DATA_PTR_ACCESS_READ, &data, &format, &stride)) {
		struct wsm_gles2_texture *tex = wsm_gles2_texture_from_pixels(wlr_renderer,
...
void wsm_gles2_texture_destroy(struct wsm_gles2_texture *texture) {
	...
	if (texture->buffer != NULL) {
		wlr_buffer_unlock(texture->buffer->base);
	} else {
		struct wsm_egl_context prev_ctx;
		wsm_egl_make_current(texture->renderer->egl, &prev_ctx);
...
struct wsm_gles2_texture *wsm_gles2_texture_from_texture(struct wlr_texture *wlr_texture) {
	assert(wlr_texture->impl == &texture_impl);
	struct wsm_gles2_texture *texture = wl_container_of(wlr_texture, texture, base);
	return texture;
}
...
struct wsm_gles2_texture *wsm_gles2_texture_from_dmabuf(
	...
	wlr_buffer_lock(texture->buffer->base);
	return texture;
