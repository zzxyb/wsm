...
void wsm_color_transform_init(struct wlr_color_transform *tr, enum wlr_color_transform_type type) {
...
struct wlr_color_transform *wlr_color_transform_init_linear_to_inverse_eotf(
	...
	wsm_color_transform_init(&tx->base, WLR_COLOR_TRANSFORM_INVERSE_EOTF);
	tx->tf = tf;
	return &tx->base;
}
...
struct wlr_color_transform *wlr_color_transform_init_lut_3x1d(size_t dim,
	...
	wsm_color_transform_init(&tx->base, WLR_COLOR_TRANSFORM_LUT_3X1D);
...
struct wlr_color_transform *wlr_color_transform_init_matrix(const float matrix[static 9]) {
	...
	wsm_color_transform_init(&tx->base, WLR_COLOR_TRANSFORM_MATRIX);
...
struct wlr_color_transform *wlr_color_transform_init_pipeline(
	...
	wsm_color_transform_init(&tx->base, WLR_COLOR_TRANSFORM_PIPELINE);
...
struct wlr_color_transform *wlr_color_transform_init_linear_to_icc(
	...
	wsm_color_transform_init(&tx->base, WLR_COLOR_TRANSFORM_LCMS2);
...
void wlr_color_transform_eval(struct wlr_color_transform *tr,
	...
	switch (tr->type) {
	case WLR_COLOR_TRANSFORM_INVERSE_EOTF:
		color_transform_inverse_eotf_eval(wsm_color_transform_inverse_eotf_from_base(tr), out, in);
		break;
	case WLR_COLOR_TRANSFORM_LCMS2:
...
void wlr_color_primaries_transform_absolute_colorimetric(
	...
	wsm_color_primaries_to_xyz(source, source_to_xyz);
	float destination_to_xyz[9];
	wsm_color_primaries_to_xyz(destination, destination_to_xyz);
