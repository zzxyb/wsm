...
static enum wsm_log_importance egl_log_importance_to_wsm(EGLint type) {
	switch (type) {
	case EGL_DEBUG_MSG_CRITICAL_KHR: return WSM_ERROR;
	case EGL_DEBUG_MSG_ERROR_KHR:    return WSM_ERROR;
	case EGL_DEBUG_MSG_WARN_KHR:     return WSM_ERROR;
	case EGL_DEBUG_MSG_INFO_KHR:     return WSM_INFO;
	default:                         return WSM_INFO;
	}
}
...
static void egl_log(EGLenum error, const char *command, EGLint msg_type,
	...
	wsm_log(egl_log_importance_to_wsm(msg_type),
...
static int get_egl_dmabuf_formats(struct wsm_egl *egl, EGLint **formats);
static int get_egl_dmabuf_modifiers(struct wsm_egl *egl, EGLint format,
...
static void log_modifier(uint64_t modifier, bool external_only) {
	...
	wsm_log(WSM_DEBUG, "    %s (0x%016"PRIX64"): ✓ texture  %s render",
		mod_name ? mod_name : "<unknown>", modifier, external_only ? "✗" : "✓");
	free(mod_name);
}

static void init_dmabuf_formats(struct wsm_egl *egl) {
	bool no_modifiers = env_parse_bool("WSM_EGL_NO_MODIFIERS");
	if (no_modifiers) {
		wsm_log(WSM_INFO, "WSM_EGL_NO_MODIFIERS set, disabling modifiers for EGL");
	}
	...
	wsm_log(WSM_DEBUG, "Supported DMA-BUF formats:");
	...
	for (int i = 0; i < formats_len; i++) {
		...
		if (wsm_log_get_verbosity() >= WSM_DEBUG) {
			char *fmt_name = drmGetFormatName(fmt);
			wsm_log(WSM_DEBUG, "  %s (0x%08"PRIX32")",
	...
	if (!no_modifiers) {
		wsm_log(WSM_DEBUG, "EGL DMA-BUF format modifiers %s",
			has_modifiers ? "supported" : "unsupported");
	}
}

static struct wsm_egl *egl_create(void) {
	const char *client_exts_str = eglQueryString(EGL_NO_DISPLAY, EGL_EXTENSIONS);
	if (client_exts_str == NULL) {
		if (eglGetError() == EGL_BAD_DISPLAY) {
			wsm_log(WSM_ERROR, "EGL_EXT_client_extensions not supported");
		} else {
			wsm_log(WSM_ERROR, "Failed to query EGL client extensions");
		}
		return NULL;
	}

	wsm_log(WSM_INFO, "Supported EGL client extensions: %s", client_exts_str);

	if (!check_egl_ext(client_exts_str, "EGL_EXT_platform_base")) {
		wsm_log(WSM_ERROR, "EGL_EXT_platform_base not supported");
		return NULL;
	}

	struct wsm_egl *egl = calloc(1, sizeof(*egl));
	if (egl == NULL) {
	...
	if (eglBindAPI(EGL_OPENGL_ES_API) == EGL_FALSE) {
		wsm_log(WSM_ERROR, "Failed to bind to the OpenGL ES API");
		free(egl);
		return NULL;
	}
...
static bool egl_init_display(struct wsm_egl *egl, EGLDisplay display,
		bool allow_software) {
	egl->display = display;

	EGLint major, minor;
	if (eglInitialize(egl->display, &major, &minor) == EGL_FALSE) {
		wsm_log(WSM_ERROR, "Failed to initialize EGL");
		return false;
	}

	const char *display_exts_str = eglQueryString(egl->display, EGL_EXTENSIONS);
	if (display_exts_str == NULL) {
		wsm_log(WSM_ERROR, "Failed to query EGL display extensions");
		return false;
	}
	...
	if (egl->exts.EXT_device_query) {
		...
		if (!egl->procs.eglQueryDisplayAttribEXT(egl->display,
			...
			wsm_log(WSM_ERROR, "eglQueryDisplayAttribEXT(EGL_DEVICE_EXT) failed");
			return false;
		}
		...
		if (device_exts_str == NULL) {
			wsm_log(WSM_ERROR, "eglQueryDeviceStringEXT(EGL_EXTENSIONS) failed");
			return false;
		}
		...
		if (check_egl_ext(device_exts_str, "EGL_MESA_device_software")) {
			if (allow_software || env_parse_bool("WSM_RENDERER_ALLOW_SOFTWARE")) {
				wsm_log(WSM_INFO, "Using software rendering");
			} else {
				wsm_log(WSM_ERROR, "Software rendering detected, please use "
					"the WSM_RENDERER_ALLOW_SOFTWARE environment variable "
					"to proceed");
				return false;
			}
		}
	...
	if (!check_egl_ext(display_exts_str, "EGL_KHR_no_config_context") &&
		...
		wsm_log(WSM_ERROR, "EGL_KHR_no_config_context or "
			"EGL_MESA_configless_context not supported");
		return false;
	}

	if (!check_egl_ext(display_exts_str, "EGL_KHR_surfaceless_context")) {
		wsm_log(WSM_ERROR, "EGL_KHR_surfaceless_context not supported");
		return false;
	}
	...
	wsm_log(WSM_INFO, "Using EGL %d.%d", (int)major, (int)minor);
	wsm_log(WSM_INFO, "Supported EGL display extensions: %s", display_exts_str);
	if (device_exts_str != NULL) {
		wsm_log(WSM_INFO, "Supported EGL device extensions: %s", device_exts_str);
	}
	wsm_log(WSM_INFO, "EGL vendor: %s", eglQueryString(egl->display, EGL_VENDOR));
	if (driver_name != NULL) {
		wsm_log(WSM_INFO, "EGL driver name: %s", driver_name);
	}
...
static bool egl_init(struct wsm_egl *egl, EGLenum platform,
	...
	if (display == EGL_NO_DISPLAY) {
		wsm_log(WSM_ERROR, "Failed to create EGL display");
		return false;
	}
	...
	if (egl->context == EGL_NO_CONTEXT) {
		wsm_log(WSM_ERROR, "Failed to create EGL context");
		return false;
	}
	...
	if (request_high_priority) {
		...
		if (priority != EGL_CONTEXT_PRIORITY_HIGH_IMG) {
			wsm_log(WSM_INFO, "Failed to obtain a high priority context");
		} else {
			wsm_log(WSM_DEBUG, "Obtained high priority context");
		}
	}
...
static EGLDeviceEXT get_egl_device_from_drm_fd(struct wsm_egl *egl,
		int drm_fd) {
	if (egl->procs.eglQueryDevicesEXT == NULL) {
		wsm_log(WSM_DEBUG, "EGL_EXT_device_enumeration not supported");
		return EGL_NO_DEVICE_EXT;
	} else if (!egl->exts.EXT_device_query) {
		wsm_log(WSM_DEBUG, "EGL_EXT_device_query not supported");
		return EGL_NO_DEVICE_EXT;
	}

	EGLint nb_devices = 0;
	if (!egl->procs.eglQueryDevicesEXT(0, NULL, &nb_devices)) {
		wsm_log(WSM_ERROR, "Failed to query EGL devices");
		return EGL_NO_DEVICE_EXT;
	}

	EGLDeviceEXT *devices = calloc(nb_devices, sizeof(*devices));
	if (devices == NULL) {
		wsm_log_errno(WSM_ERROR, "Failed to allocate EGL device list");
		return EGL_NO_DEVICE_EXT;
	}
	...
	if (drm_fd >= 0) {
		...
		if (ret < 0) {
			wsm_log(WSM_ERROR, "Failed to get DRM device: %s", strerror(-ret));
			free(devices);
			return EGL_NO_DEVICE_EXT;
		}
	}
	...
	for (int i = 0; i < nb_devices; i++) {
		...
		if (check_egl_ext(device_exts_str, "EGL_EXT_device_drm")) {
			...
			if (egl_device_name == NULL) {
				wsm_log(WSM_ERROR, "eglQueryDeviceStringEXT(EGL_DRM_DEVICE_FILE_EXT) failed");
				continue;
			}
		}
		...
		if (found) {
			if (egl_device_name != NULL) {
				wsm_log(WSM_DEBUG, "Using EGL device %s", egl_device_name);
			}
...
static int open_render_node(int drm_fd) {
	...
	if (render_name == NULL) {
		...
		if (render_name == NULL) {
			wsm_log_errno(WSM_ERROR, "drmGetPrimaryDeviceNameFromFd failed");
			return -1;
		}
		wsm_log(WSM_DEBUG, "DRM device '%s' has no render node, "
			"falling back to primary node", render_name);
	}

	int render_fd = open(render_name, O_RDWR | O_CLOEXEC);
	if (render_fd < 0) {
		wsm_log_errno(WSM_ERROR, "Failed to open DRM node '%s'", render_name);
	}
	free(render_name);
	return render_fd;
}
	...
	if (egl->exts.EXT_platform_device) {
		...
		if (egl_device != EGL_NO_DEVICE_EXT) {
			if (egl_init(egl, EGL_PLATFORM_DEVICE_EXT, egl_device, allow_software)) {
				wsm_log(WSM_DEBUG, "Using EGL_PLATFORM_DEVICE_EXT");
				return egl;
			}
			goto error;
		}
		...
		wsm_log(WSM_DEBUG, "EXT_platform_device not supported");
	}

	if (egl->exts.KHR_platform_gbm && drm_fd >= 0) {
		int gbm_fd = open_render_node(drm_fd);
		if (gbm_fd < 0) {
			wsm_log(WSM_ERROR, "Failed to open DRM render node");
			goto error;
		}
		...
		if (!egl->gbm_device) {
			close(gbm_fd);
			wsm_log(WSM_ERROR, "Failed to create GBM device");
			goto error;
		}

		if (egl_init(egl, EGL_PLATFORM_GBM_KHR, egl->gbm_device, allow_software)) {
			wsm_log(WSM_DEBUG, "Using EGL_PLATFORM_GBM_KHR");
			return egl;
		}
		...
		wsm_log(WSM_DEBUG, "KHR_platform_gbm not supported");
	}

error:
	wsm_log(WSM_ERROR, "Failed to initialize EGL context");
	free(egl);
	eglReleaseThread();
	return NULL;
}

struct wsm_egl *wsm_egl_create_with_context(EGLDisplay display,
		EGLContext context) {
	EGLint client_type;
	if (!eglQueryContext(display, context, EGL_CONTEXT_CLIENT_TYPE, &client_type) ||
			client_type != EGL_OPENGL_ES_API) {
		wsm_log(WSM_ERROR, "Unsupported EGL context client type (need OpenGL ES)");
		return NULL;
	}
	...
	if (!eglQueryContext(display, context, EGL_CONTEXT_CLIENT_VERSION, &client_version) ||
			client_version < 2) {
		wsm_log(WSM_ERROR, "Unsupported EGL context client version (need OpenGL ES >= 2)");
		return NULL;
	}
...
bool wsm_egl_destroy_image(struct wsm_egl *egl, EGLImage image) {
...
bool wsm_egl_make_current(struct wsm_egl *egl,
		struct wsm_egl_context *save_context) {
	...
	if (!eglMakeCurrent(egl->display, EGL_NO_SURFACE, EGL_NO_SURFACE,
			egl->context)) {
		wsm_log(WSM_ERROR, "eglMakeCurrent failed");
		return false;
	}
	return true;
}
...
EGLImageKHR wsm_egl_create_image_from_dmabuf(struct wsm_egl *egl,
	...
	if (!egl->exts.KHR_image_base || !egl->exts.EXT_image_dma_buf_import) {
		wsm_log(WSM_ERROR, "dmabuf import extension not present");
	...
	if (attributes->modifier != DRM_FORMAT_MOD_INVALID &&
			...
			!egl->has_modifiers) {
		wsm_log(WSM_ERROR, "EGL implementation doesn't support modifiers");
		return EGL_NO_IMAGE_KHR;
	}
	...
	if (image == EGL_NO_IMAGE_KHR) {
		wsm_log(WSM_ERROR, "eglCreateImageKHR failed");
		return EGL_NO_IMAGE_KHR;
	}
...
static int get_egl_dmabuf_formats(struct wsm_egl *egl, EGLint **formats) {
	if (!egl->exts.EXT_image_dma_buf_import) {
		wsm_log(WSM_DEBUG, "DMA-BUF import extension not present");
		return -1;
	}
	...
	if (!egl->procs.eglQueryDmaBufFormatsEXT(egl->display, 0, NULL, &num)) {
		wsm_log(WSM_ERROR, "Failed to query number of dmabuf formats");
		return -1;
	}

	*formats = calloc(num, sizeof(**formats));
	if (*formats == NULL) {
		wsm_log(WSM_ERROR, "Allocation failed: %s", strerror(errno));
		return -1;
	}

	if (!egl->procs.eglQueryDmaBufFormatsEXT(egl->display, num, *formats, &num)) {
		wsm_log(WSM_ERROR, "Failed to query dmabuf format");
		free(*formats);
		return -1;
	}
	return num;
...
static int get_egl_dmabuf_modifiers(struct wsm_egl *egl, EGLint format,
	...
	*external_only = NULL;
	...
	if (!egl->exts.EXT_image_dma_buf_import) {
		wsm_log(WSM_DEBUG, "DMA-BUF extension not present");
		return -1;
	}
	if (!egl->exts.EXT_image_dma_buf_import_modifiers) {
		return 0;
	...
	if (!egl->procs.eglQueryDmaBufModifiersEXT(egl->display, format, 0,
			NULL, NULL, &num)) {
		wsm_log(WSM_ERROR, "Failed to query dmabuf number of modifiers");
		return -1;
	}
	if (num == 0) {
		return 0;
	...
	*external_only = calloc(num, sizeof(**external_only));
	...
	if (!egl->procs.eglQueryDmaBufModifiersEXT(egl->display, format, num,
		...
		wsm_log(WSM_ERROR, "Failed to query dmabuf modifiers");
		free(*modifiers);
		free(*external_only);
		return -1;
	}
...
const struct wlr_drm_format_set *wsm_egl_get_dmabuf_texture_formats(
		struct wsm_egl *egl) {
	return &egl->dmabuf_texture_formats;
}

const struct wlr_drm_format_set *wsm_egl_get_dmabuf_render_formats(
		struct wsm_egl *egl) {
...
static char *get_render_name(const char *name) {
	...
	if (devices_len < 0) {
		wsm_log(WSM_ERROR, "drmGetDevices2 failed: %s", strerror(-devices_len));
		return NULL;
	}
	...
	if (match == NULL) {
		wsm_log(WSM_ERROR, "Cannot find DRM device %s", name);
...
static int dup_egl_device_drm_fd(struct wsm_egl *egl) {
	...
	if (egl->exts.EXT_device_drm_render_node) {
		...
		if (name == NULL) {
			wsm_log(WSM_DEBUG, "EGL device has no render node");
			return -1;
		}
	...
	if (render_name == NULL) {
		...
		if (render_name == NULL) {
			wsm_log(WSM_ERROR, "Can't find render node name for device %s",
				primary_name);
			return -1;
		}
	}
	...
	if (render_fd < 0) {
		wsm_log_errno(WSM_ERROR, "Failed to open DRM render node %s",
			render_name);
		free(render_name);
		return -1;
	}
...
int wsm_egl_dup_drm_fd(struct wsm_egl *egl) {
	...
	if (fd < 0) {
		wsm_log_errno(WSM_ERROR, "Failed to dup GBM FD");
	}
	return fd;
}
...
EGLSyncKHR wsm_egl_create_sync(struct wsm_egl *egl, int fence_fd) {
	...
	if (fence_fd >= 0) {
		...
		if (dup_fd < 0) {
			wsm_log_errno(WSM_ERROR, "dup failed");
			return EGL_NO_SYNC_KHR;
		}
	...
	if (sync == EGL_NO_SYNC_KHR) {
		wsm_log(WSM_ERROR, "eglCreateSyncKHR failed");
		if (dup_fd >= 0) {
			close(dup_fd);
		}
	}
...
void wsm_egl_destroy_sync(struct wsm_egl *egl, EGLSyncKHR sync) {
	...
	if (egl->procs.eglDestroySyncKHR(egl->display, sync) != EGL_TRUE) {
		wsm_log(WSM_ERROR, "eglDestroySyncKHR failed");
	}
}
...
int wsm_egl_dup_fence_fd(struct wsm_egl *egl, EGLSyncKHR sync) {
	...
	if (fd == EGL_NO_NATIVE_FENCE_FD_ANDROID) {
		wsm_log(WSM_ERROR, "eglDupNativeFenceFDANDROID failed");
		return -1;
	}
...
bool wsm_egl_wait_sync(struct wsm_egl *egl, EGLSyncKHR sync) {
	if (egl->procs.eglWaitSyncKHR(egl->display, sync, 0) != EGL_TRUE) {
		wsm_log(WSM_ERROR, "eglWaitSyncKHR failed");
		return false;
	}
	return true;
}
